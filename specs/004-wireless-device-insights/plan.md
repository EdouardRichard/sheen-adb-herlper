...
- LAN 选择接线在 manager 策略实现前增加公共契约步骤：`WirelessDiscoveryModels.kt` 定义携带 generation 与 observationId 的不透明选择引用，`AdbSessionManager.kt` 定义默认结构化拒绝且必清除 secret 的 discovered pairing/connect 方法；Feature 不把地址或端口拼成 endpoint。随后 `DefaultAdbSessionManager.kt` 与 reducer 才实现重新确认、attempt 关联和可选 verified identity 回填，继续保持每任务最多两个文件。
